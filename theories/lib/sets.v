...
(** This file provides lemmas on finite sets [gset].
  * The stdpp library provides a definitions and lemmas on finite sets [gset],
  * but states lemmas axiomatically in terms of the [FinSet] typeclass, which
  * may make them hard to read for novice users of stdpp. We therefore
  * instantiate the most important lemmas specifically for [gset] to make the
  ...
  * The most important operations are:
    ...
    - the intersection operation X ∩ Y represents the intersection of two sets
    - the difference operation X ∖ Y represents the difference
      (Note: this is NOT a backslash. Pay attention to use the right unicode 
      symbol.)
    - the subset predicate X ⊆ Y states that X is a subset of Y
    - the element predicate x ∈ X states that x is an element of X
    - the elements operation elements X gives a duplicate-free list of elements
      of X

  * stdpp's sets satisfy extensional Leibniz equality -- that is, two sets X and
  * Y are equal (X = Y) iff they contain the same elements. *)
