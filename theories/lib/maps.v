From Intro2TT.lib Require Export sets.
From stdpp Require Import gmap.
From stdpp Require Export relations orders.

(** * Finite maps *)
(** This file provides lemmas on finite maps (functions with a finite domain) 
  * [gmap]. The stdpp library provides a definitions and lemmas on finite maps
  * [gmap], but states lemmas axiomatically in terms of the [FinMap] typeclass,
  * which may make them hard to read for novice users of stdpp. We therefore
  * instantiate the most important lemmas specifically for [gmap] to make the
  * statements easier to comprehend.

  * The most important operations are:
    - the empty map ∅ contains no elements
    - the singleton map {[ i := v ]} defines a singleton map that maps key i to
      value v
    - the insert operation <[ i := v ]> m inserts the association i ↦ v to the
      map m, overriding any mappings for i in m
    - the delete operation delete i m deletes the mapping for key i from m
    - the lookup operation m !! i lookups a key i, and yields the optional value
      associated with i
    - the fmap operation f <$> m maps a function f over all values stored in the
      map
    - the subset relation m1 ⊆ m2 states that all associations of map m1 are
      also included in map m2
    - the dom operation dom m yields a finite set ([gset]) of the domain of the
      map

  * stdpp's maps satisfy extensional Leibniz equality -- that is, two maps X and
  * Y are equal (X = Y) iff they represent the same finite function. *)
